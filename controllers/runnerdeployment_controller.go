...
func (r *RunnerDeploymentReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if !ok {
		log.Info("Failed to get template hash of runnerset resource. It must be in an invalid state. Please manually delete the runnerset so that it is recreated")
...
func getTemplateHash(rs *v1alpha1.RunnerSet) (string, bool) {
	hash, ok := rs.Labels["runner-template-hash"]
